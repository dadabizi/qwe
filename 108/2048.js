/*
	author:108小组 18/11/11
*/

let sgypGame = {
    start: function() {
        this.data = [];
        for (let i = 0; i < this.Heng; i++) {
            this.data[i] = [];
            for (let j = 0; j < this.Shu; this.data[i][j] = 0, j++){}
        } //将data做一个4*4矩阵数组
        this.someNextNum();
        //获取随机数
        this.someNextNum();
        ...
        document.addEventListener('keydown', (e) => {  //添加键盘事件监听,上下左右箭头
            switch (e.keyCode) {                              //(keyCode37 38 39 40 为上下左右键)
                case 37:
                    this.goLeft();
                    break;
                case 38:
    ...
    move: function(callback) {
        let move1 = String(this.data); //把数组字符串化，进行操作
        callback(); //进行一次回调函数，用arrow function，this指向最近的对象,匿名函数回调指向window对象
        let move2 = String(this.data);
        if (move1 !== move2) {
    ...
    resetFlex: function() {
        ...
        for (let i = 0; i < this.Heng; i++) {
            for (let j = 0; j < this.Shu; j++) {
                ...
                let oDiv = document.getElementById("item" + i + j);
    ...
    Logic: function(event) {
        ...
        document.onmouseup = function(event) {
            ...
            let scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;
            let scrollY = document.documentElement.scrollTop || document.body.scrollTop;
            let x1 = e.pageX || e.clientX + scrollX;
            let y1 = e.pageY || e.clientY + scrollY;
    ...
    gameOver: function() {
        for (let h = 0; h < this.Heng; h++) {
            for (let s = 0; s < this.Shu; s++) { //双重嵌套最为致命
    ...
    someNextNum: function() {
        //算出随机数,如果随机的格子为空，赋值
        let h = Math.floor(Math.random() * (this.Heng));
        let s = Math.floor(Math.random() * (this.Shu));
        if (this.data[h][s] === 0) {
    ...
    goLeftNext: function(h) {
        for (let s = 0; s < this.Shu - 1; s++) {
            let nextShu = this.getLeftNext(h, s); //获取本行下一个块块（for循环判断是否存在）
            if (nextShu === -1) {
                break;
            } //如果没有下一个块块，就不用判断了。
            else {
    ...
    goRightNext: function(h) { //本行下一个块，赋值替换操作
        for (let s = this.Shu - 1; s > 0; s--) { //从最大值开始递减，有块就操作，没块就赋值
            let prevShu = this.getRightNext(h, s); //从2开始判断到0，最右侧不用管
            if (prevShu === -1) {
                break;
            } //没有块块就最好啦！！
            else {
    ...
    goTop: function() {
        this.move(() => {
            for (let s = 0; s < this.Shu; s++) {
                this.goTopNext(s);
            }
        });
    },
    goTopNext: function(s) {
        for (let h = 0; h < this.Heng - 1; h++) { //和上面差不多，改为从纵向判断
            let nextHeng = this.getTopNext(h, s);
            if (nextHeng === -1) {
                break;
            } else {
                if (this.data[h][s] === 0) {
    ...
    goBottomNext: function(s) {
        for (let h = this.Heng - 1; h > 0; h--) {
            let prevHeng = this.getBottomNext(h, s);
            if (prevHeng === -1) {
                break;
            } else {
                if (this.data[h][s] === 0) {
